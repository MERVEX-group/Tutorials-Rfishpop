...
<span style="color:red">NOTE: Our package does not implement a generic function implementing a HCR is not available yet but is coming.</span> 
...
Using Sum.Pop.Mod we can check if effectively the catches for 2021 is 1500.
...
```{r}
Pop.Mod1$Info$ctrFish$f
```


As we mentioned before we can check also the effect of reducing the effort any percentage. For example, the next line extend the Pop.Mod object reducing the effort to a 60% of the effort in the previous year 2020.
...
```{r}
Pop.Mod2$Info$ctrFish$f
```


We can do the same of several years directly as follows.
...
We can also use tol	argument to increase the level of tolerence in the optimization process.
